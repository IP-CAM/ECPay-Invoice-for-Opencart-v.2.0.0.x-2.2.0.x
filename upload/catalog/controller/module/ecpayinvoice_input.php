...
?>
	...
	<div class="invoice_info"  style="display:none;" ><input type="text" name="company_write" id="company_write" value="" placeholder="統一編號" class="form-control"></div>
	...
	$("#button-payment-method").click(function(){
		...
		else
		{
			...
			$.ajax({
				type: 'POST',
				url: 'http://<?=$_SERVER['HTTP_HOST']?><?=$_SERVER['PHP_SELF']?>/index.php?route=payment/ecpayinvoice/set_invoice_info',
				dataType: 'html',
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$.ajax({
			type: 'POST',
			url: 'http://<?=$_SERVER['HTTP_HOST']?><?=$_SERVER['PHP_SELF']?>/index.php?route=payment/ecpayinvoice/del_invoice_info',
			dataType: 'html',
