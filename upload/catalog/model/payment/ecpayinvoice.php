<?php
class ModelPaymentecpayinvoice extends Model {
	...
	{
		$sFile_Name =  dirname(dirname(dirname(dirname(__FILE__)))) . DIRECTORY_SEPARATOR.'admin'.DIRECTORY_SEPARATOR.'controller'.DIRECTORY_SEPARATOR.'payment'.DIRECTORY_SEPARATOR.'Ecpay_Invoice.php' ;
	...
	{	
		...
		if(!$bError)
		{
			...
			else
			{
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$aOrder_Info_Tmp['order_status_id'] . "', notify = '0', comment = '" . $this->db->escape($sMsg) . "', date_added = NOW()");
				$this->db->query("DELETE FROM `invoice_info` WHERE `order_id` = " . $order_id );
			}	
